...
def _c(ca,i,j,P,Q):
    if ca[i,j] > -1:
        ...
        ca[i,j] = max(_c(ca,i-1,0,P,Q),euc(P[i],Q[0]))
    elif i == 0 and j > 0:
        ca[i,j] = max(_c(ca,0,j-1,P,Q),euc(P[0],Q[j]))
    elif i > 0 and j > 0:
        ca[i,j] = max(min(_c(ca,i-1,j,P,Q),_c(ca,i-1,j-1,P,Q),_c(ca,i,j-1,P,Q)),euc(P[i],Q[j]))
    else:
        ca[i,j] = float("inf")
    return ca[i,j]
